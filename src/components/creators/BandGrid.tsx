...
// travelling shine. Extracted from CtaBand on 2026-09-09 when the expectations dialog asked for the
// same lines at its own scale (Appy: "something similar to what is there on the onBlue website").
// CtaBand's own header had already said it: the geometry "is the whole file and must not exist
// twice", so a second copy at dialog size was the one thing not to do.
//
...
//
// TWO THINGS ARE PARAMETERS, and only two. `idPrefix`, because two grids on one page with the same
// mask and gradient ids render correctly right up until one of them changes size — and there are
// two on a page the moment the dialog opens over the closer. And `shine`, because the travelling
// wave belongs to a full-width band you scroll past; behind a dialog's three cards it is a
// distraction competing with the thing it frames.
//
// THIS IS THE onBlue FILE'S TWIN, NOT A SHARED MODULE, and the difference is the shine: that fork
// dropped the brand gradient, so its stops are one flat blue. Here the wave is still iris -> cyan.
// The two trees are deliberately separate (see the fork note in CreatorsHome); a shared component
// whose only variable is the thing the forks disagree about is not shared, it is branched.
//
