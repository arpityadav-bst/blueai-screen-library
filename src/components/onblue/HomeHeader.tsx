...
// The /onblue header — Phase 1 of PLAN.md, rewired for Phase 3's creator flow. The mock shipped
// headerless; this is the first deliberately-added piece, styled in the page's OWN language (see
// onblue.css's header block for the fixed-not-sticky and z-index reasoning).
//
...
// simply visible at every width instead; onblue.css's header block carries the measured fit.
//
  ...
  return (
    <header className={`crx-header ${scrolled ? 'scrolled' : ''}`}>
      <div className="crx-header-in">
        <button type="button" className="crx-logo" aria-label="onBlue Partners, top of page" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
          <span className="crx-logo-col">
            <span ref={wordmarkWrapRef}>
              <OnBlueWordmark size={19} />
            </span>
            <span
              ref={labelRef}
