...
// carries no word the B/C variants ban, so the per-variant terms machinery collapses to one copy.
// The outreach clause still reads the variant: only Version A may say "the program".
// The link opens a NEW TAB — this checkbox sits on the last step of an eleven-question form, and
// in-place navigation here would cost the reader all of it.
function AgreeCopy() {
  const { variant } = useCrx()
  return (
    <>
      By applying, I agree to the{' '}
      <a className="crx-legal-link" href="/onblue/terms#terms" target="_blank" rel="noopener">
        Terms of Use
      </a>{' '}
      and I&apos;m okay with BlueStacks reaching out to me about{' '}
      {variant === 'programs' ? 'the program' : 'onBlue'}.
    </>
  )
}
