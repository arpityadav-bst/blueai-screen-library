...
2026-08-11 — blueai-desktop.css — `.bai-ui.wide .bai-tour { top: 46px }`, the same sibling bug, fixed but FLAGGED as not making the tour work in landscape — Why: `tourHi()` targets `.bai-tab` and the card's arrow reads `tab.offsetLeft`, both dead when the strip is `display:none`; the tour is compact-only by construction and a sidebar-aware tour is a design decision, not a CSS one. Fixed the arithmetic because a correct scrim is never worse, flagged so the half-fix can't read as done.
